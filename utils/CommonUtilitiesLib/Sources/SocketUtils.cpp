...
#error
    ...
    for (ifReqIter = buffer; ifReqIter < (buffer + ifc.ifc_len);)
    {
        ...
        if (!IncrementIfReqIter(&ifReqIter, ifr))
            return;
...
#ifndef __Win32__
Bool16 IncrementIfReqIter(char** inIfReqIter, ifreq* ifr)
{
...
UInt32   SocketUtils::GetIPAddr(UInt32 inAddrIndex)
{
...
StrPtrLen*   SocketUtils::GetIPAddrStr(UInt32 inAddrIndex)
{
	Assert(sIPAddrInfoArray != NULL);
	Assert(inAddrIndex < sNumIPAddrs);
...
StrPtrLen*   SocketUtils::GetDNSNameStr(UInt32 inDNSIndex)
{
	Assert(sIPAddrInfoArray != NULL);
...
UInt32       SocketUtils::GetNumIPAddrs() { return sNumIPAddrs; }
