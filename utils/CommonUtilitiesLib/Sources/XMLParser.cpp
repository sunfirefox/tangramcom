...
static inline char _upper_char( char ch )
{
    if (ch >= 'a' && ch <= 'z') {
        return ch - 'a' + 'A';
    } else {
...
static int _strcmp_nc (const char * src,const char * dst)
{
    int ret = 0 ;

    while( ! (ret = _upper_char(*(unsigned char *)src) - _upper_char(*(unsigned char *)dst)) && *dst)
        ++src, ++dst;
...
UInt8 XMLTag::sNonNameMask[] =
{
    ...
    0, 0, 0, 0, 1, 0, 1, 1, 1, 1, //120-129 modify by txf
...
char* XMLTag::GetAttributeValue(const char* attrName)
{
    for (OSQueueIter iter(&fAttributes); !iter.IsDone(); iter.Next())
    {
        ...
        if (!_strcmp_nc(attr->fAttrName, attrName))
...
XMLTag* XMLTag::GetEmbeddedTagByName(const char* tagName, const UInt32 index)
{
    ...
    for (OSQueueIter iter(&fEmbeddedTags); !iter.IsDone(); iter.Next())
    {
        ...
        if (!_strcmp_nc(temp->GetTagName(), tagName))
        {
            if (curIndex == index)
            {
                result = temp;
...
XMLTag* XMLTag::GetEmbeddedTagByAttr(const char* attrName, const char* attrValue, const UInt32 index)
{
    ...
    {
        XMLTag* temp = (XMLTag*)iter.GetCurrent()->GetEnclosingObject();
        if ((temp->GetAttributeValue(attrName) != NULL) && (!_strcmp_nc(temp->GetAttributeValue(attrName), attrValue)))
        {
            if (curIndex == index)
            {
                result = temp;
...
XMLTag* XMLTag::GetEmbeddedTagByNameAndAttr(const char* tagName, const char* attrName, const char* attrValue, const UInt32 index)
{
    ...
    {
        XMLTag* temp = (XMLTag*)iter.GetCurrent()->GetEnclosingObject();
        if (!_strcmp_nc(temp->GetTagName(), tagName) && (temp->GetAttributeValue(attrName) != NULL) && 
            (!_strcmp_nc(temp->GetAttributeValue(attrName), attrValue)))
        {
            if (curIndex == index)
            {
                result = temp;
...
void XMLTag::SetAttribute(char* attrName, char* attrValue)
{
