...
class TGMCOMUTI_API UDPDemuxerUtils
{
    private:
...
class TGMCOMUTI_API UDPDemuxerTask
{
    public:
...
class TGMCOMUTI_API UDPDemuxerKey
{
    private:
...
class  UDPDemuxer
{
    ...
    public:
       ...
       TGMCOMUTI_API UDPDemuxer() : fHashTable(kMaxHashTableSize), fMutex() {}
       TGMCOMUTI_API ~UDPDemuxer() {}
       ...
       TGMCOMUTI_API OS_Error RegisterTask(UInt32 inRemoteAddr, UInt16 inRemotePort,
                                        UDPDemuxerTask *inTaskP);

        // Return values: OS_NoErr, or EPERM if this task / address combination
        // is not registered
       TGMCOMUTI_API OS_Error UnregisterTask(UInt32 inRemoteAddr, UInt16 inRemotePort,
                                        UDPDemuxerTask *inTaskP);
        
        //Assumes that parent has grabbed the mutex!
       TGMCOMUTI_API UDPDemuxerTask* GetTask(UInt32 inRemoteAddr, UInt16 inRemotePort);

       TGMCOMUTI_API Bool16  AddrInMap(UInt32 inRemoteAddr, UInt16 inRemotePort)
                    { return (this->GetTask(inRemoteAddr, inRemotePort) != NULL); }
                    
       TGMCOMUTI_API OSMutex*                GetMutex()      { return &fMutex; }
      TGMCOMUTI_API  UDPDemuxerHashTable*    GetHashTable()  { return &fHashTable; }
