...
#ifdef __Win32__
#if _MSC_VER >= 1301 //MSVC71 for bug,must preinclude winsock2.h before windows.h
...
:   OSEventContext(OSEventContext::kInvalidFileDesc, NULL),
    fState(inSocketType),
    ...
    fPortStr(fPortBuffer, kPortBufSizeInBytes)
{
	...
	//Socket::Initialize();
	//SetEventThread(sEventThread);
    //OSEventThread::GetDefaultEventThread()->SetAutoStop(false);
    SetEventThread(OSEventThread::GetDefaultEventThread());
...
OS_Error Socket::Bind(UInt32 addr, UInt16 port)
{
    ...
    fLocalAddr.sin_addr.s_addr = htonl(addr);//0 是合法的
...
//static int sEventThreadRefCount = 0;


//void Socket::Initialize() 
//{
	//OSMutexLocker _locker(EventThread::GetMutex());

	//if (sEventThreadRefCount++ == 0)
	//{
...
//{ 
	...
	//if (--sEventThreadRefCount == 0)
	//{
	//	EventThread *pThread = sEventThread;
	//	sEventThread = NULL;
	//	delete pThread;
	//}
	
	//Assert(sEventThreadRefCount >= 0);
//}


//void Socket::StartThread() 
//{
	//sEventThread->StartSelect(); 
//}

//void Socket::StopThread()
//{
	//sEventThread->StopSelect();
//}


//EventThread* Socket::GetEventThread() { return sEventThread; }

UInt32  Socket::GetLocalAddr()  { return ntohl(fLocalAddr.sin_addr.s_addr); }
UInt16  Socket::GetLocalPort()  { return ntohs(fLocalAddr.sin_port); }
