...
#elif defined(REGISTER_PROXY_DLL) && defined(_EXPORT_STANDARD_PROXY_DLLROUTINES_)
# define DLLREGISTERSERVER_ENTRY	DECLSPEC_EXPORT DllRegisterServer 
# define DLLUNREGISTERSERVER_ENTRY	DECLSPEC_EXPORT DllUnregisterServer 
# define DLLMAIN_ENTRY				DECLSPEC_EXPORT DllMain 
# define DLLGETCLASSOBJECT_ENTRY	DECLSPEC_EXPORT DllGetClassObject 
# define DLLCANUNLOADNOW_ENTRY		DECLSPEC_EXPORT DllCanUnloadNow 
...
#define REGISTER_PROXY_DLL_ROUTINES(pfl, factory_clsid) \
    ...
    { \
		if(hProxyDll == NULL) hProxyDll =  GetCurrentModule((void*)__get_cur_module_func_addr__);/*add by txf*/ \
