...
#define BASE_TYPE_UNMARSHALL(type) \
   ...
   case RPC_FC_FLOAT:
    ...
    case RPC_FC_HYPER:
        ...
        TRACE("value: %s\n", tgmlog_dbgstr_longlong(*(ULONGLONG *)pMemory));
        break;
    case RPC_FC_ENUM16:
    ...
    PFORMAT_STRING pFormat)
{
  ...
  case RPC_FC_C_WSTRING:
    ...
    {
      TRACE("string=%s\n", TGMLOG_STR_A((const char *)pMemory));
      pStubMsg->ActualCount = strlen((const char *)pMemory)+1;
    }
    else
    {
      TRACE("string=%s\n", TGMLOG_STR_W((LPCWSTR)pMemory));
  ...
  case RPC_FC_C_WSTRING:
    ...
    if (fUnmarshall)
    {
      ...
      if (*pFormat == RPC_FC_C_CSTRING)
        TRACE("string=%s\n", TGMLOG_STR_A((char*)*ppMemory));
      else
        TRACE("string=%s\n", TGMLOG_STR_W((LPWSTR)*ppMemory));
    }
    return bufsize;
  default:
    ERR("unknown array format 0x%x\n", fc);
                                ...
                                unsigned char fMustAlloc)
{
  ...
  if (*pFormat == RPC_FC_CSTRING) {
    TRACE("string=%s\n", TGMLOG_STR_AN((char*)*ppMemory, pStubMsg->ActualCount));
  }
  else if (*pFormat == RPC_FC_WSTRING) {
    TRACE("string=%s\n", TGMLOG_STR_WN((LPWSTR)*ppMemory, pStubMsg->ActualCount));
  }
                                       ...
                                       PFORMAT_STRING pPointer)
{
  ...
  while (*pFormat != RPC_FC_END) {
    ...
    case RPC_FC_ENUM32:
      ...
      break;
    case RPC_FC_HYPER:
      TRACE("longlong=%s <= %p\n", tgmlog_dbgstr_longlong(*(ULONGLONG*)pMemory), pMemory);
                                ...
                                unsigned char fMustAlloc)
{
    ...
    if (*pCVArrayFormat == RPC_FC_C_CSTRING)
        TRACE("string=%s\n", TGMLOG_STR_A((char *)(*ppMemory + pCVStructFormat->memory_size)));
    else if (*pCVArrayFormat == RPC_FC_C_WSTRING)
        TRACE("string=%s\n", TGMLOG_STR_W((WCHAR *)(*ppMemory + pCVStructFormat->memory_size)));
...
#define BASE_TYPE_UNMARSHALL(type) \
   ...
   case RPC_FC_FLOAT:
    ...
    case RPC_FC_HYPER:
        BASE_TYPE_UNMARSHALL(ULONGLONG);
        TRACE("value: %s\n", tgmlog_dbgstr_longlong(**(ULONGLONG **)ppMemory));
        break;
    case RPC_FC_ENUM16:
        ALIGN_POINTER(pStubMsg->Buffer, sizeof(USHORT));
        if (fMustAlloc || !*ppMemory)
