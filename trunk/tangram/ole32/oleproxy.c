...
{
    *ppv = NULL;
	if (IsEqualIID(rclsid,&CLSID_DfMarshal)		&&	(IsEqualIID(iid,&IID_IClassFactory) ||	IsEqualIID(iid,&IID_IUnknown))    ){
		TRACE("MARSHAL_GetStandardMarshalCF \n");
	...
	if (IsEqualIID(rclsid,&CLSID_StdGlobalInterfaceTable) && (IsEqualIID(iid,&IID_IClassFactory) || IsEqualIID(iid,&IID_IUnknown))){
		TRACE("StdGlobalInterfaceTable_GetFactory \n");
