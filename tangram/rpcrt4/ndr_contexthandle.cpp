...
static CRITICAL_SECTION ndr_context_cs = { NULL, -1, 0, 0, 0, 0 };
...
static struct context_handle_entry *context_entry_from_guid(LPCGUID uuid)
{
	...
	{
		const IID* uuid0 = &che->wire_data.uuid;
        if (IsEqualGUID(uuid0, uuid))
            return che;
	}
    return NULL;
}
                                      ...
                                      const ndr_context_handle *chi)
{
	...
	const IID* uuid0 = &chi->uuid;
	const IID* uuid1 = &GUID_NULL;
    /* a null UUID means we should free the context handle */
    if (IsEqualGUID(uuid0, uuid1))
    {
        if (*CContext)
        {
            che = get_context_entry(*CContext);
    ...
    {
        che = (struct context_handle_entry *)HeapAlloc(GetProcessHeap(), 0, sizeof *che);
        if (!che)
                                  ...
                                  void *pBuff, ULONG DataRepresentation)
{
    ...
    EnterCriticalSection(&ndr_context_cs);
    r = ndr_update_context_handle(CContext, hBinding, (ndr_context_handle *)pBuff);
    LeaveCriticalSection(&ndr_context_cs);
    if (r)
        RpcRaiseException(r);
}
...
//{
    //RpcBinding *binding = (RpcBinding *)hBinding;
    //RPC_STATUS status;
    //ndr_context_handle *ndr = (ndr_context_handle *)pBuff;
...
//{
    ...
    NDR_SCONTEXT SContext;
    RPC_STATUS status;
    const ndr_context_handle *context_ndr = (const ndr_context_handle *)pBuff;
