...
{	
	...
	* 由ServerRpc实现
	*/
	class CRpcUnixConnectionRequestObj:public IUnknownImp<ITgmRpcConnectionRequestObject>
	{
	public:
		/*!
		* 获取请求/通知序号
		...
		*/
		...
		//////////////////////////////////////////////////////////////////////////
		CRpcUnixConnectionRequestObj(RpcComPacket * inPacket,CRpcUnixConnection* inConn)
			:mPacketPtr(inPacket),mConnection(inConn)	
		{
			Assert(inPacket);
		}
        
		virtual ~CRpcUnixConnectionRequestObj(){}

	private:
		CComSmartPtr<RpcComPacket>		mPacketPtr;
		CComSmartPtr<CRpcUnixConnection>	mConnection;
	};
	/*!
	* 服务器通知对象接口
	* 由ClientRpc实现
	*/
	class RpcUnixConnectionNotifyObj:public IUnknownImp<ITgmRpcConnectionNotifyObject>
	{
	public:
		//////////////////////////////////////////////////////////////////////////
		//! 获取请求/通知序号
		...
		{return mPacketPtr->data_size;}
		//////////////////////////////////////////////////////////////////////////
		RpcUnixConnectionNotifyObj(RpcComPacket *inPacket):mPacketPtr(inPacket)
		{

		}

		virtual ~RpcUnixConnectionNotifyObj()
		{
			TRACE("~RpcUnixConnectionNotifyObj() %p,%X\r\n",this,GetRef());
		}
	...
	CRpcUnixConnection::CRpcUnixConnection(BOOL isServer /* = FALSE */, BOOL isSimple /* = FALSE */)
		:Task(DEFAULT_TASK_THREAD_POOL_INS)
        , mIsSvrConn(FALSE)
		, mSocket(NULL, UnixSocket::kBlockingSocketType)
		, mProtseq(&mSocket)
		, mIsConnected(FALSE)
		, mIsServer(isServer)
		, mIsSimple(isSimple)
	{
		...
		sprintf(theTaskName,"CRpcUnixConnection%p",this);
	...
	CRpcUnixConnection::~CRpcUnixConnection()
	{
		TRACE("~CRpcUnixConnection() %p,%X\r\n",this,GetRef());
		Assert(0 == GetRef());
		Disconnect();
	...
	HRESULT CRpcUnixConnection::Connect(const char* inPath)
	{
		...
		{
			WARN("CRpcUnixConnection::Connect Failed (%s)\n",strerror(theErr) );
			mSocket.Cleanup();
		}
	...
	*  
	*/
	HRESULT CRpcUnixConnection::Disconnect()
	{
		if(mIsConnected)
		{
			mSocket.Cleanup();
	...
	HRESULT	CRpcUnixConnection::DisconnectUnexpected()
	{
		if(mIsConnected)
		{
			mSocket.Cleanup();
	...
	*
	*/
	HRESULT CRpcUnixConnection::Advise(ITgmRpcConnectionEvent * inEventHandler,ULONG inIsSvrConn)
	{
		TRACE("CRpcUnixConnection::Advise() mEvtHandler:%p,inEventHandler:%p,inIsSvrConn(%d)\n"
	...
	*
	*/
	HRESULT CRpcUnixConnection::Unadvise()
	{
		mEvtHandler = NULL;
		return S_OK;
	}
	...
	HRESULT CRpcUnixConnection::WaitResult(RpcComPacket** ioPacket)
	{
		int nSelRet;
		fd_set fdsRead;
	...
	*  
	*/
	HRESULT CRpcUnixConnection::SendSyncRequest(const void *inParamsPtr,ULONG inParmsSize,
	...
	HRESULT CRpcUnixConnection::SendSyncRequestEx(const void *inParamsPtr,ULONG inParmsSize,
	...
	*/
	HRESULT CRpcUnixConnection::SendAsyncRequest(const void *inParamsPtr
		, ULONG inParmsSize
	...
	*  
	*/
	HRESULT CRpcUnixConnection::SendNotify(const void *inParamsPtr,ULONG inParmsSize)
	{
		ULONG theSeqNo = mIsServer?RpcComProtocol::gen_server_sequno():RpcComProtocol::gen_client_seqno();
	...
	HRESULT CRpcUnixConnection::SendReply(ULONG inSequNo,const void *inParamsPtr,ULONG inParmsSize)
	{	
	...
	HRESULT CRpcUnixConnection::SendResult(ULONG inSequNo,HRESULT inResult)
	{	
	...
	HRESULT CRpcUnixConnection::GetResourceLocation(char* outBindString,ULONG inBingStringSize)
	{
		CComSmartPtr<RpcResourceLoactionIf> theRRL = new CRpcResourceLoaction();
		if(theRRL)
		{
			theRRL->set_ProtSequ((const char*)RPCBINGING_PROTSEQU_RPCCOM_UNIX);
			theRRL->set_NetworkAddr(mSocket.GetLocalPath());
		...
		if (event & Task::kReadEvent)
		{
			...
			if(SUCCEEDED(hr))	
			{
				...
				case RPCCOM_METHOD_REQUEST:
					if(mEvtHandler)
					{
						CComSmartPtr<ITgmRpcConnectionRequestObject> theReqPtr = new CRpcUnixConnectionRequestObj(thePacket,this);
				...
				case RPCCOM_METHOD_NOTIFY:
					if(mEvtHandler)
					{
						CComSmartPtr<ITgmRpcConnectionNotifyObject> theNtyPtr =  new RpcUnixConnectionNotifyObj(thePacket);
			...
			else
			{
				...
				case RPCCOM_E_CONN_PIPE:
					DisconnectUnexpected();//断开所有链接
					TRACE("DisconnectUnexpected for hr:0x%X\n",hr);
					break;
				case RPCCOM_E_SOCKET_AGAIN:
					mSocket.RequestEvent(OSEventContext::EV_RE);//继续去读数据
					break;
	...
	HRESULT CRpcUnixConnection::AddRequest(ULONG inSeqNo,ClientRequestObject *inReqPtr)
	{
	...
	HRESULT CRpcUnixConnection::GetRequest(ULONG inSeqNo,ClientRequestObject** outObj)
	{
		OSMutexLocker _locker(&mMutex);
	...
	void CRpcUnixConnection::CancelRequest(ULONG inSeqNo)
	{
		GetRequest(inSeqNo,NULL);
	}

	void	CRpcUnixConnection::CancelAllRequest(HRESULT inResult)
	{
		OSMutexLocker _locker(&mMutex);
