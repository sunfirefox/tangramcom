...
//#ifdef _TXF_MODIFY_
//extern DWORD sThreadStorageIndex;
//#endif

extern struct oletls * glo_rpcrt4_tls;

static inline struct oletls *COM_CurrentInfo(void)
{
	if(!glo_rpcrt4_tls)
	{
		glo_rpcrt4_tls = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(struct oletls));
	}
	return glo_rpcrt4_tls;
//#ifdef _TXF_MODIFY_
//	struct oletls * tls = TlsGetValue(sThreadStorageIndex);
//	if(!tls)
//	{
//		tls = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(struct oletls));
//		TlsSetValue(sThreadStorageIndex,tls);
//	}
//	return tls;
//#else
