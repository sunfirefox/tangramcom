...
{
    const func_t *func;
    const char *implicit_handle = (const char *)get_attrp(iface->attrs, ATTR_IMPLICIT_HANDLE);
    const var_t *var;
    int method_count = 0;
    ...
    if (iface->funcs) LIST_FOR_EACH_ENTRY( func, iface->funcs, const func_t, entry )
    {
        ...
        const char *callconv = (const char *)get_attrp(def->type->attrs, ATTR_CALLCONV);
...
static void write_clientinterfacedecl(type_t *iface)
{
    ...
    const UUID *uuid = (const UUID *)get_attrp(iface->attrs, ATTR_UUID);
    const str_list_t *endpoints = (const str_list_t *)get_attrp(iface->attrs, ATTR_ENDPOINT);
