...
extern "C" {
    ...
    WINADVAPI	HANDLE WINAPI Agr_CreateMutex();
    WINADVAPI	void WINAPI Agr_LockMutex(HANDLE handle);
    WINADVAPI	void WINAPI Agr_UnlockMutex(HANDLE handle);
    WINADVAPI	void WINAPI Agr_CloseMutex(HANDLE handle);
...
static inline HRESULT get_facbuf_for_iid(REFIID riid, IPSFactoryBuffer **facbuf)
{
    ...
    hr = CoGetClassObject(&clsid, CLSCTX_INPROC_SERVER | WINE_CLSCTX_DONT_HOST,
...
HRESULT marshal_object(APARTMENT *apt, STDOBJREF *stdobjref, REFIID riid, IUnknown *object, MSHLFLAGS mshlflags)
{
    ...
    * the wire */
    if (!IsEqualIID(riid, &IID_IUnknown))
    {
        ...
        if (hr != S_OK)
        {
            ERR("couldn't get IPSFactory buffer for interface %s,hr 0x%X\n", TGMLOG_STR_GUID(riid),hr);
...
static HRESULT WINAPI ClientIdentity_QueryInterface(IMultiQI * iface, REFIID riid, void ** ppv)
{
    ...
    mqi.pIID = riid;
    hr = IMultiQI_QueryMultipleInterfaces(iface, 1, &mqi);//ClientIdentity_AddRef
    *ppv = (void *)mqi.pItf;
...
static HRESULT WINAPI ClientIdentity_QueryMultipleInterfaces(IMultiQI *iface, ULONG cMQIs, MULTI_QI *pMQIs)
{
    ...
    * interfaces and also interfaces exposed by the proxy manager */
    for (i = 0; i < cMQIs; i++)
    {
        TRACE("iid[%d] = %s\n", i, TGMLOG_STR_GUID(pMQIs[i].pIID));
    ...
    * to query the remote object for it */
        ...
        * of the proxy_manager so we use the correct ipid here? */
        ...
        if (SUCCEEDED(hr))
        {
            hr = IRemUnknown_RemQueryInterface(remunk, ipid, NORMALEXTREFS,
                (unsigned short)nonlocal_mqis, iids, &qiresults);
    ...
    FE_TRACE("< %d/%d successfully queried\n", successful_mqis, cMQIs);
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            TRACE("writing stdobjref:\n\tflags = %04lx\n\tcPublicRefs = %ld\n\toxid = %s\n\toid = %s\n\tipid = %s\n",
                ...
                tgmlog_dbgstr_longlong(stdobjref.oxid),
                tgmlog_dbgstr_longlong(stdobjref.oid),
                TGMLOG_STR_GUID(&stdobjref.ipid));
                                             ...
                                             DWORD Capabilities)
{
    FIXME("(%p, %d, %d, %s, %d, %d, %p, 0x%x): stub\n", pProxy, AuthnSvc,
        AuthzSvc, TGMLOG_STR_W(pServerPrincName), AuthnLevel, ImpLevel,
...
static HRESULT ifproxy_get_public_ref(struct ifproxy * This)
{
    ...
    //    return E_UNEXPECTED;
    //}
    Agr_LockMutex(This->parent->remoting_mutex);

    FB_TRACE("> begin getting public ref for ifproxy %p,%d\n", This,(int)This->refs);
    if (This->refs == 0)
    {
    ...
    Agr_UnlockMutex(This->parent->remoting_mutex);
    FE_TRACE("< end getting public ref for ifproxy %p\n", This);
...
static HRESULT ifproxy_release_public_refs(struct ifproxy * This)
{
    ...
    FB_TRACE("> releasing for ifproxy %p,%d\n", This,(int)This->refs);
    Agr_LockMutex(This->parent->remoting_mutex);
    ...
    if (public_refs > 0)
    {
        ...
        if (hr == S_OK)
        {
            ...
            if (hr == S_OK)
                ...
                tgmlog_dbgstr_longlong(This->parent->oxid),
                tgmlog_dbgstr_longlong(This->parent->oid));
            else
    ...
    Agr_UnlockMutex(This->parent->remoting_mutex);
    FE_TRACE("< releasing for ifproxy %p,%d\n", This,(int)This->refs);
...
static HRESULT proxy_manager_get_remunknown_copy(struct proxy_manager * This,APARTMENT * apt)
{
    struct list * cursor;
    assert(!This->remunk);
    LIST_FOR_EACH(cursor, &apt->proxies)
    {
        ...
        if ((This->oxid == proxy->oxid) && (proxy->remunk))
        {
            This->remunk = proxy->remunk;
            IRemUnknown_AddRef(proxy->remunk);
            return S_OK;
        }
    }
    return E_FAIL;
                                       ...
                                       const OXID_INFO *oxid_info, struct proxy_manager ** proxy_manager)
{
    ...
    This->remoting_mutex =  Agr_CreateMutex();
    ...
    else
    {
        ...
        if (FAILED(hr))
        {
            ...
            Agr_CloseMutex(This->remoting_mutex);
            HeapFree(GetProcessHeap(), 0, This);
            return hr;
        }
    }
    ...
    list_init(&This->entry);		// entry in apartment (CS parent->cs) 
    list_init(&This->interfaces);	// imported interfaces (CS cs) 
    ...
    * overwritten in proxy_manager_set_context */
    ...
    //add by txf
    proxy_manager_get_remunknown_copy(This,apt);
    ...
    * IRemUnknown proxies that need late destruction */
    ...
    FE_TRACE("< %p created for OXID %s, OID %s\n", This,
        tgmlog_dbgstr_longlong(oxid), tgmlog_dbgstr_longlong(oid));
    ...
    IRpcChannelBuffer * channel, struct ifproxy ** iif_out)
{
    ...
    * proxy associated with the ifproxy as we handle IUnknown ourselves */
    ...
    {
        hr = get_facbuf_for_iid(riid, &psfb);//riid根本不起作用，这里就是为了获得OLE32实现的PSFactoryBuffer
        if (hr == S_OK)
        {
    ...
    if (hr == S_OK)
    {
        ...
        *iif_out = ifproxy;
        TRACE("ifproxy %p created for IPID %s, interface %s with %lu public refs\n",
            ifproxy, TGMLOG_STR_GUID(&stdobjref->ipid), TGMLOG_STR_GUID(riid), stdobjref->cPublicRefs);
    }
    else
...
static void proxy_manager_disconnect(struct proxy_manager * This)
{
    ...
    TRACE("oxid = %s, oid = %s\n", tgmlog_dbgstr_longlong(This->oxid),
        tgmlog_dbgstr_longlong(This->oid));
...
static HRESULT proxy_manager_create_remunknown(struct proxy_manager * This,IRemUnknown **remunk)
{
    STDOBJREF stdobjref;
    HRESULT hr;
...
//    else
//    {
//          hr = proxy_manager_create_remunknown(This,remunk);
//    }
//    LeaveCriticalSection(&This->cs);
//
//    FE_TRACE("< got IRemUnknown* pointer %p, hr = 0x%08x\n", *remunk, hr);
//
//    return hr;
}
...
static HRESULT proxy_manager_get_remunknown(struct proxy_manager * This, IRemUnknown **remunk)
{
    ...
    * lifetime management */
    ...
    //>>add by txf 这一行其实不需要，因为最早在工厂创建的时候，都已经创建了remunknown 对象
    //if(!This->remunk)
    //     proxy_manager_get_remunknown_copy(This,apt);
    //<<
                                ...
                                void **object)
{
    ...
    FB_TRACE("> *** stdobjref:flags = %04lxcPublicRefs = %ldoxid = %soid = %sipid = %s\n",
        stdobjref->flags, stdobjref->cPublicRefs,
        tgmlog_dbgstr_longlong(stdobjref->oxid),
        tgmlog_dbgstr_longlong(stdobjref->oid),
        TGMLOG_STR_GUID(&stdobjref->ipid));
...
StdMarshalImpl_UnmarshalInterface(LPMARSHAL iface, IStream *pStm, REFIID riid, void **ppv)
{
    ...
    if ((oxid == stdobjref.oxid) && (stubmgr = get_stub_manager(apt, stdobjref.oid)))
    {
        TRACE("Unmarshalling object marshalled in same apartment for iid %s, "
            "returning original object %p\n", TGMLOG_STR_GUID(riid), stubmgr->object);
    ...
    * up returning RPC_E_DISCONNECTED. */
    //check local OXID list.if not in the list,then from other process
...
StdMarshalImpl_ReleaseMarshalData(LPMARSHAL iface, IStream *pStm)
{
    ...
    if (!(stubmgr = get_stub_manager(apt, stdobjref.oid)))
    {
        ERR("could not map object ID to stub manager, oxid=%s, oid=%s\n",
            tgmlog_dbgstr_longlong(stdobjref.oxid), tgmlog_dbgstr_longlong(stdobjref.oid));
        return RPC_E_INVALID_OBJREF;
    }
                                    ...
                                    DWORD mshlflags, LPMARSHAL *ppMarshal)
{
    ...
    if (pUnk == NULL)
    {
        FIXME("(%s,NULL,%x,%p,%x,%p), unimplemented yet.\n",
            TGMLOG_STR_GUID(riid),dwDestContext,pvDestContext,mshlflags,ppMarshal);
        return E_NOTIMPL;
    }
    TRACE("(%s,%p,%x,%p,%x,%p)\n",
        TGMLOG_STR_GUID(riid),pUnk,dwDestContext,pvDestContext,mshlflags,ppMarshal);
...
static HRESULT get_unmarshaler_from_stream(IStream *stream, IMarshal **marshal, IID *iid)
{
    ...
    {
        ERR("Failed to read common OBJREF header, 0x%08x,res(%lu)\n", hr,res);
                                  ...
                                  DWORD mshlFlags)
{
    ...
    FB_TRACE("> (%p, %s, %p, %x, %p,", pStream, TGMLOG_STR_GUID(riid), pUnk,
    ...
    if (IsEqualCLSID(&marshaler_clsid, &CLSID_DfMarshal))
    {
        ...
        if (hr)
        {
            ...
            TRACE("Marsaler marshaler_clsid(%s) OBJREF(%s),length(%d)\n",
                TGMLOG_STR_GUID(&marshaler_clsid),TGMLOG_STR_AN((const char*)&objref, FIELD_OFFSET(OBJREF, u_objref)),FIELD_OFFSET(OBJREF, u_objref));
        }
    }
    else
    {
...
cleanup:
    ...
    FE_TRACE("< completed with hr 0x%08x\n", hr);
...
HRESULT WINAPI CoUnmarshalInterface(IStream *pStream, REFIID riid, LPVOID *ppv)
{
    ...
    FB_TRACE("> (%p, %s, %p)\n", pStream, TGMLOG_STR_GUID(riid), ppv);

    if (!pStream || !ppv)
        return E_INVALIDARG;

    hr = get_unmarshaler_from_stream(pStream, &pMarshal, &iid);//根据marsghalled数据来创建Marshaler
    if (hr != S_OK)
        return hr;
    ...
    FE_TRACE("< completed with hr 0x%x\n", hr);
...
//    HRESULT		hres;
//
//    TRACE("(%s, %p, %p)\n",TGMLOG_STR_GUID(riid), pUnk, ppStm);
//
