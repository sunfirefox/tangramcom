...
struct oletls * glo_rpcrt4_tls = NULL;
...
//{
//#ifdef _TXF_MODIFY_
//    struct oletls *info = TlsGetValue(sThreadStorageIndex);
//#else
...
//#ifdef _TXF_MODIFY_
//		TlsSetValue(sThreadStorageIndex,NULL);
//#else
...
static APARTMENT *apartment_construct(DWORD model)
{
    ...
    apt->tid = (GetCurrentProcessId()+GetCurrentThreadId()+(DWORD)apt) & 0xFFFF;//modify by txf
    ...
    TRACE("Created apartment on OXID= %s,tid= %X,apt=%p\n", tgmlog_dbgstr_longlong(apt->oxid),(unsigned int)apt->tid,apt);
...
DWORD apartment_addref(struct apartment *apt)
{
    ...
    TRACE("%s: before = %d\n", tgmlog_dbgstr_longlong(apt->oxid), refs - 1);
    return refs;
}
...
DWORD apartment_release(struct apartment *apt)
{
    ...
    TRACE("%s: after = %d\n", tgmlog_dbgstr_longlong(apt->oxid), ret);
    ...
    if (ret == 0)
    {
        ...
        TRACE("destroying apartment %p, oxid %s\n", apt, tgmlog_dbgstr_longlong(apt->oxid));
	...
	//txf add
//	 if (apt->remunk) IRemUnknown_Release(apt->remunk);
...
{
//#ifdef _TXF_MODIFY_
	//static const WCHAR wszOle32_win[] = {'C',':','\\','W','I','N','D','O','W','S','\\','s','y','s','t','e','m','3','2','\\','o','l','e','3','2','.','d','l','l',0};
	//c:\windows\system32\ole32.dll
//#endif
	static const WCHAR wszOle32[] = {'t','g','m','o','l','e','3','2','.','d','l','l',0};//tgmole32
	...
	TRACE("dllpath:%s,rclsid:%s,riid:%s,wszOle32:%s\r\n",TGMLOG_STR_W(dllpath),TGMLOG_STR_GUID(rclsid),TGMLOG_STR_GUID(riid),TGMLOG_STR_W(wszOle32));

//#ifdef _TXF_MODIFY_
//	if (!strcmpiW(dllpath, wszOle32_win) || !strcmpiW(dllpath, wszOle32) )
//#else
...
//#endif
    {
        ...
        TRACE("calling tgmole32!DllGetClassObject\n");
...
HRESULT WINAPI CoInitializeEx(LPVOID lpReserved, DWORD dwCoInit)
{
  ...
  dwCoInit &= ~COINIT_APARTMENTTHREADED;//add by txf
...
WINADVAPI INT WINAPI agr_strncpy_ctow(LPWSTR dst, LPCSTR src ,int n);
...
HRESULT WINAPI StringFromCLSID(REFCLSID id, LPOLESTR *idstr)
{
	...
	if (ret == S_OK) {
            ...
            *idstr = IMalloc_Alloc( mllc, len * sizeof(WCHAR) );
			...
			agr_strncpy_ctow(*idstr,buf,len);
	}
	return ret;
}
...
INT WINAPI StringFromGUID2(REFGUID id, LPOLESTR str, INT cmax)
{
  ...
  return agr_strncpy_ctow(str,xguid,cmax);
}
...
HRESULT WINAPI CoGetPSClsid(REFIID riid, CLSID *pclsid)
{
    ...
    FB_TRACE("> () riid=%s, pclsid=%p\n", TGMLOG_STR_GUID(riid), pclsid);

    if (!apt)
    {
        FE_ERR("< apartment not initialised\n");
        return CO_E_NOTINITIALIZED;
    }

    if (!pclsid)
    {
        FE_ERR("< pclsid isn't optional\n");
        return E_INVALIDARG;
    }
    ...
    {
        FE_WARN("< No PSFactoryBuffer object is registered for IID %s\n", TGMLOG_STR_GUID(riid));
    ...
    {
		RegCloseKey(hkey);
        FE_WARN("< () CoGetPSClsid RegQueryValueW Failed= %s\n", TGMLOG_STR_W(value));
        return REGDB_E_IIDNOTREG;
    }
    RegCloseKey(hkey);
	...
	if (CLSIDFromString(value, pclsid) != NOERROR){
		FE_WARN("< () CLSIDFromString Failed CLSID=%s\n", TGMLOG_STR_W(value));
        return REGDB_E_IIDNOTREG;
	}

    FE_TRACE("< () Returning CLSID=%s\n", TGMLOG_STR_GUID(pclsid));
    return S_OK;
}
...
HRESULT WINAPI CoRegisterPSClsid(REFIID riid, REFCLSID rclsid)
{
    ...
    FB_TRACE("> (%s, %s)\n", TGMLOG_STR_GUID(riid), TGMLOG_STR_GUID(rclsid));
    ...
    LPDWORD lpdwRegister)
{
  ...
  FB_TRACE("> (%s,%p,0x%08x,0x%08x,%p)\n",
	TGMLOG_STR_GUID(rclsid),pUnk,dwClsContext,flags,lpdwRegister);
  ...
  if (!apt)
  {
      FE_ERR("< COM was not initialized\n");
      return CO_E_NOTINITIALIZED;
  }
  ...
  *lpdwRegister = newClass->dwCookie;
  ...
  if (dwClsContext & CLSCTX_LOCAL_SERVER) {
      ...
      if (hr) {
          FE_FIXME("< Failed to create stream on hglobal, %x\n", hr);
          return hr;
      }
      ...
      if (hr) {
          FE_FIXME("< CoMarshalInterface failed, %x!\n",hr);
          return hr;
      }
    ...
    if (curClass->dwCookie == dwRegister)
    {
      ...
      else
      {
          ERR("called from wrong apartment, should be called from %s\n",
              tgmlog_dbgstr_longlong(curClass->apartment_id));
...
static DWORD COM_RegReadPath(HKEY hkeyroot, const WCHAR *keyname, const WCHAR *valuename, WCHAR * dst, DWORD dstlen)
{
...
#if 0//modify by txf; our registry not support NULL keyName
	...
	if(keyname == NULL){
		if( (ret = RegQueryValueExW(hkeyroot, NULL, NULL, &keytype, (LPBYTE)src, &dwLength)) == ERROR_SUCCESS ) {
			if (keytype == REG_EXPAND_SZ) {
				//if (dstlen <= ExpandEnvironmentStringsW(src, dst, dstlen)) ret = ERROR_MORE_DATA;
				lstrcpynW(dst, src, dstlen);//FIX ME
			} else {
				lstrcpynW(dst, src, dstlen);
			}
		}
	}else if((ret = RegOpenKeyExW(hkeyroot, keyname, 0, KEY_READ, &key)) == ERROR_SUCCESS) {
		if( (ret = RegQueryValueExW(key, NULL, NULL, &keytype, (LPBYTE)src, &dwLength)) == ERROR_SUCCESS ) {
			if (keytype == REG_EXPAND_SZ) {
				//if (dstlen <= ExpandEnvironmentStringsW(src, dst, dstlen)) ret = ERROR_MORE_DATA;
				lstrcpynW(dst, src, dstlen);//FIX ME
                                       ...
                                       BOOL hostifnecessary, void **ppv)
{
    ...
    //                TGMLOG_STR_W(threading_model), TGMLOG_STR_GUID(rclsid));
    ...
    {
        /* failure: CLSID is not found in registry */
        FE_WARN("< class %s not registered inproc\n", TGMLOG_STR_GUID(rclsid));
        return REGDB_E_CLASSNOTREG;
    }

    hres = apartment_getclassobject(apt, dllpath, apartment_threaded,
                                    rclsid, riid, ppv);
    FE_TRACE("< \n");
    return hres;
}
    ...
    REFIID iid, LPVOID *ppv)
{
    ...
    FB_TRACE(">*** CLSID:%s,IID:%s\n", TGMLOG_STR_GUID(rclsid), TGMLOG_STR_GUID(iid));
    ...
    *ppv = NULL;
    ...
    if (pServerInfo) {
	    FIXME("\tpServerInfo: name=%s\n",TGMLOG_STR_W(pServerInfo->pwszName));
                                             ...
                                             &regClassObject))
    {
      ...
      FE_TRACE("< COM_GetRegisteredClassObject CLSID:%s,IID:%s,ppv=%p,hres=%x\n", TGMLOG_STR_GUID(rclsid), TGMLOG_STR_GUID(iid),ppv,hres);
    ...
    if (CLSCTX_INPROC_SERVER & dwClsContext)
    {
        ...
        if (FAILED(hres))
        {
            if (hres == REGDB_E_CLASSNOTREG)
                TRACE("class %s not registered\n", TGMLOG_STR_GUID(rclsid));
            else if (hres == REGDB_E_KEYMISSING)
            {
                TRACE("class %s not registered as in-proc server\n", TGMLOG_STR_GUID(rclsid));
         ...
         * other types */
        if (SUCCEEDED(hres)){
            FE_TRACE("< CLSCTX_INPROC_SERVER CLSID:%s,IID:%s,ppv=%p,hres=%x\n", TGMLOG_STR_GUID(rclsid), TGMLOG_STR_GUID(iid),ppv,hres);
            return hres;
        }
    }
    ...
    if (CLSCTX_INPROC_HANDLER & dwClsContext)
    {
        ...
        if (FAILED(hres))
        {
            ...
            {
                TRACE("class %s not registered in-proc handler\n", TGMLOG_STR_GUID(rclsid));
                hres = REGDB_E_CLASSNOTREG;
            }
        }
         ...
         * other types */
        if (SUCCEEDED(hres)){
            FE_TRACE("< CLSCTX_INPROC_HANDLER CLSID:%s,IID:%s,ppv=%p,hres=%x\n", TGMLOG_STR_GUID(rclsid), TGMLOG_STR_GUID(iid),ppv,hres);
            return hres;
        }
    }
    ...
    if (CLSCTX_LOCAL_SERVER & dwClsContext)
    {
        ...
        if (SUCCEEDED(hres)){
            FE_TRACE("< CLSCTX_LOCAL_SERVER CLSID:%s,IID:%s,ppv=%p,hres=%x\n", TGMLOG_STR_GUID(rclsid), TGMLOG_STR_GUID(iid),ppv,hres);
            return hres;
        }
    }
    ...
    if (FAILED(hres))
        ERR("no class object %s could be created for context 0x%x\n",
            TGMLOG_STR_GUID(rclsid), dwClsContext);
    FE_TRACE("< hr=0x%x\n",hres);
    return hres;
}
	...
	LPVOID *ppv)
{
  ...
  FB_TRACE("> (rclsid=%s, pUnkOuter=%p, dwClsContext=%08x, riid=%s, ppv=%p)\n", TGMLOG_STR_GUID(rclsid),
        pUnkOuter, dwClsContext, TGMLOG_STR_GUID(iid), ppv);
  ...
  *ppv = 0;
  ...
  if (FAILED(hres))
  {
    FE_TRACE("< failed (rclsid=%s,riid=%s, ppv=%p, hres=%x)\n", TGMLOG_STR_GUID(rclsid),TGMLOG_STR_GUID(iid), ppv,hres);
    return hres;
  }
   ...
   */
    ...
    TRACE("- IClassFactory_CreateInstance hres=%x\n",hres);
	...
	if(FAILED(hres))
        {
          if (hres == CLASS_E_NOAGGREGATION && pUnkOuter)
              ...
              FIXME("no instance created for interface %s of class %s, hres is 0x%08x\n", TGMLOG_STR_GUID(iid), TGMLOG_STR_GUID(rclsid),hres);
        }

    FE_TRACE("< (rclsid=%s,riid=%s, ppv=%p, hres=%x)\n", TGMLOG_STR_GUID(rclsid),TGMLOG_STR_GUID(iid), ppv,hres);
	return hres;
}
  ...
  MULTI_QI*     pResults)
{
  ...
  if (successCount!=cmq){
    FE_TRACE("< CO_S_NOTALLINTERFACES\n");
...
//{
//    TRACE("(%s, %d)\n", TGMLOG_STR_W(lpszLibName), bAutoFree);
//
    ...
    BOOL fLastUnlockReleases)
{
    ...
    FB_TRACE("> pUnk=%p, fLock=%s, fLastUnlockReleases=%s\n",
...
HRESULT WINAPI CoGetTreatAsClass(REFCLSID clsidOld, LPCLSID clsidNew)
{
    ...
    FIXME("(%s,%p)\n", TGMLOG_STR_GUID(clsidOld), clsidNew);
...
//{
//    TRACE("(%s, %p)\n", TGMLOG_STR_GUID(guidExtension), pChannelHook);
//
...
//{
//    FIXME("(%s): stub\n", TGMLOG_STR_GUID(logical_thread_id));
//    return E_NOTIMPL;
//}
//
