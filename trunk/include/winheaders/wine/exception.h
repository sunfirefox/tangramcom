 ...
 */
	...
	#ifdef _MSC_VER
	//txf modify
	#define __EXCEPT(func) __except((func)(GetExceptionCode(),GetExceptionInformation()))
...
#elif defined( USE_PROGRAM_JUMP )  /* USE_COMPILER_EXCEPTIONS */
...
//		sigjmp_buf jmp;
	...
	#define __TRY if (1) {
	#define __EXCEPT(func) } else {/*__asm int 3h;*/
	#define __FINALLY(func) }{
	#define __ENDTRY }
	#define __EXCEPT_PAGE_FAULT 
	#define __EXCEPT_ALL } else {/*__asm int 3h;*/
