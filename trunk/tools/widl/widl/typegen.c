...
static int is_base_type(unsigned char type)
{
    switch (type)
    {
    case RPC_FC_BYTE:
...
void assign_stub_out_args( FILE *file, int indent, const func_t *func, const char *local_var_prefix )
{
    ...
    LIST_FOR_EACH_ENTRY( var, func->args, const var_t, entry )
    {
        ...
        if (!in_attr)
        {
			...
			else if (is_string)//add by txf
			{
				//仅仅是编译通过，我觉得实现还是有问题.txf
				fprintf(file, " ;/*need modify.FIXME TXF*/\n");
			}
...
{
#ifdef WIDL_PS_EXCEPTION_DISABLE
