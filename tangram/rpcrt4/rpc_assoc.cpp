...
static CRITICAL_SECTION assoc_list_cs = { NULL, -1, 0, 0, 0, 0 };
...
{
    RpcAssoc *assoc;
    assoc = (RpcAssoc *)HeapAlloc(GetProcessHeap(), 0, sizeof(*assoc));
    if (!assoc)
                                          ...
                                          const RPC_SYNTAX_IDENTIFIER *TransferSyntax)
{
	...
	void * res_buf = NULL;
	ULONG res_len = 0;
	ipc_client_memory_free_fun_t res_free;
	...
	HRESULT hr = conn->rpc_connection->SendSyncRequestEx(hdr,hdr->common.frag_len,&res_buf,&res_len,&res_free,0);
	...
	response_hdr = theUnPacket.get_Header();
	theUnPacket.get_RpcMessage(&msg);
	
	switch (response_hdr->common.ptype)
    {
    case PKT_BIND_ACK:
    {
        RpcAddressString *server_address = (RpcAddressString *)msg.Buffer;
                ...
                case RESULT_PROVIDER_REJECTION:
                    ...
                    case REASON_ABSTRACT_SYNTAX_NOT_SUPPORTED:
                        ERR("syntax %s, %d.%d not supported\n",
                            TGMLOG_STR_GUID(&InterfaceId->SyntaxGUID),
...
void RpcAssoc_ReleaseIdleConnection(RpcAssoc *assoc, RpcConnection *Connection)
{
    ...
    EnterCriticalSection(&assoc->cs);
    if (!assoc->assoc_group_id) 
        assoc->assoc_group_id = Connection->assoc_group_id;
...
//              TGMLOG_STR_GUID(&context_handle->uuid), context_handle);
//    }
//    LeaveCriticalSection(&assoc->cs);
//
//    return RPC_S_OK;
