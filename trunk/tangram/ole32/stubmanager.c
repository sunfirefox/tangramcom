...
static void stub_manager_delete(struct stub_manager *m)
{
    ...
    FB_TRACE("> destroying %p (oid=%s)\n", m, tgmlog_dbgstr_longlong(m->oid));
...
struct stub_manager *get_stub_manager(APARTMENT *apt, OID oid)
{
    ...
    if (result)
        TRACE("found %p for oid %s\n", result, tgmlog_dbgstr_longlong(oid));
    else
        TRACE("not found for oid %s\n", tgmlog_dbgstr_longlong(oid));
...
HRESULT ipid_to_stub_manager(const IPID *ipid, APARTMENT **stub_apt, struct stub_manager **stubmgr_ret)
{
    ...
    FB_TRACE("> %s IRemUnknown\n",(ipid->Data2 == 0xffff)?"is":"isnot");
    ...
    if (!*stub_apt)
    {
        FE_TRACE("< Couldn't find apartment corresponding to TID 0x%04x\n", ipid->Data2);
...
struct ifstub *stub_manager_new_ifstub(struct stub_manager *m, IRpcStubBuffer *sb, IUnknown *iptr, REFIID iid, MSHLFLAGS flags)
{
    ...
    FB_TRACE("> oid=%s, stubbuffer=%p, iptr=%p, iid=%s\n",
        tgmlog_dbgstr_longlong(m->oid), sb, iptr, TGMLOG_STR_GUID(iid));
    ...
    * for the remunknown interface */
    ...
    FE_TRACE("< ifstub %p created with ipid %s\n", stub, TGMLOG_STR_GUID(&stub->ipid));
...
static void stub_manager_delete_ifstub(struct stub_manager *m, struct ifstub *ifstub)
{
    FB_TRACE("> m=%p, m->oid=%s, ipid=%s\n", m, tgmlog_dbgstr_longlong(m->oid), TGMLOG_STR_GUID(&ifstub->ipid));
...
BOOL stub_manager_notify_unmarshal(struct stub_manager *m, const IPID *ipid)
{
    ...
    ERR("stub_manager_notify_unmarshal IPID %s\n", TGMLOG_STR_GUID(ipid));//fellow code invalid .txf recommend 
...
{
    TRACE("(%p)->(%s, %p)\n", iface, TGMLOG_STR_GUID(riid), ppv);
                                                   ...
                                                   REMQIRESULT **ppQIResults /* [size_is(,cIids)] */)
{
    ...
    FB_TRACE("> (%p)->(%s, %d, %d, %p, %p)\n", iface, TGMLOG_STR_GUID(ripid), cRefs, cIids, iids, ppQIResults);
