 ...
 * Copyright (C) 2009  Jokul for Tranzda
 *
...
TGMLOG_DEFAULT_DEBUG_CHANNEL(ntdll);
 ...
 */
void WINE_WINAPI RtlInitAnsiString(
 ...
 *  of source is assumed to be 0.
 */
NTSTATUS WINE_WINAPI RtlInitAnsiStringEx(PANSI_STRING target, PCSZ source)
{
    if (source)
    {
 ...
 *  of source is assumed to be 0.
 */
void WINE_WINAPI RtlInitString(
 ...
 */
void WINE_WINAPI RtlFreeAnsiString( PSTRING str )
{
    if (str->Buffer)
    {
        free( str->Buffer );
 ...
 */
void WINE_WINAPI RtlFreeOemString( PSTRING str )
{
 ...
 *	RtlCopyString   (NTDLL.@)
 */
void WINE_WINAPI RtlCopyString( STRING *dst, const STRING *src )
{
    if (src)
    {
 ...
 *  of source is assumed to be 0.
 */
void WINE_WINAPI RtlInitUnicodeString(
 ...
 *  of source is assumed to be 0.
 */
NTSTATUS WINE_WINAPI RtlInitUnicodeStringEx(
    PUNICODE_STRING target, /* [I/O] Buffered unicode string to be initialized */
    PCWSTR source)          /* [I]   '\0' terminated unicode string used to initialize target */
{
    if (source != NULL) {
 ...
 *     Failure: FALSE
 */
BOOLEAN WINE_WINAPI RtlCreateUnicodeString( PUNICODE_STRING target, LPCWSTR src )
{
    int len = (strlenW(src) + 1) * sizeof(WCHAR);
    if (!(target->Buffer = malloc( len ))) return FALSE;
 ...
 *     Failure: FALSE
 */
BOOLEAN WINE_WINAPI RtlCreateUnicodeStringFromAsciiz( PUNICODE_STRING target, LPCSTR src )
{
    STRING ansi;
 ...
 *     nothing
 */
void WINE_WINAPI RtlFreeUnicodeString( PUNICODE_STRING str )
{
    if (str->Buffer)
    {
        free( str->Buffer );
 ...
 *     nothing
 */
void WINE_WINAPI RtlCopyUnicodeString( UNICODE_STRING *dst, const UNICODE_STRING *src )
{
    if (src)
    {
        unsigned int len = min( src->Length, dst->MaximumLength );
 ...
 */
NTSTATUS WINE_WINAPI RtlDuplicateUnicodeString(
    int add_nul,                  /* [I] flag */
    ...
    UNICODE_STRING *destination)  /* [O] destination for the duplicated unicode string */
{
    if (source == NULL || destination == NULL ||
        ...
        if (source->Length == 0 && add_nul != 3) {
            ...
            destination->Buffer = malloc( destination_max_len);
 ...
 *     nothing
 */
void WINE_WINAPI RtlEraseUnicodeString( UNICODE_STRING *str )
{
    if (str->Buffer)
    {
 ...
 */
LONG WINE_WINAPI RtlCompareString( const STRING *s1, const STRING *s2, BOOLEAN CaseInsensitive )
{
    unsigned int len;
    LONG ret = 0;
    LPCSTR p1, p2;
 ...
 */
LONG WINE_WINAPI RtlCompareUnicodeString( const UNICODE_STRING *s1, const UNICODE_STRING *s2,
 ...
 */
BOOLEAN WINE_WINAPI RtlEqualString( const STRING *s1, const STRING *s2, BOOLEAN CaseInsensitive )
{
 ...
 */
BOOLEAN WINE_WINAPI RtlEqualUnicodeString( const UNICODE_STRING *s1, const UNICODE_STRING *s2,
                                      BOOLEAN CaseInsensitive )
{
    if (s1->Length != s2->Length) return FALSE;
 ...
 */
BOOLEAN WINE_WINAPI RtlPrefixString( const STRING *s1, const STRING *s2, BOOLEAN ignore_case )
{
    unsigned int i;
 ...
 */
BOOLEAN WINE_WINAPI RtlPrefixUnicodeString( const UNICODE_STRING *s1,
 ...
 */
//WCHAR WINE_WINAPI RtlAnsiCharToUnicodeChar(LPSTR *ansi)
//{
//    WCHAR str;
 ...
 */
NTSTATUS WINE_WINAPI RtlAnsiStringToUnicodeString(
 ...
 */
NTSTATUS WINE_WINAPI RtlUnicodeStringToAnsiString(
 ...
 */
NTSTATUS WINE_WINAPI RtlMultiByteToUnicodeN( LPWSTR dst, DWORD dstlen, LPDWORD reslen,
                                        LPCSTR src, DWORD srclen )
{

	CHAR *p = src;
	while(srclen && dstlen && (*dst = *p) != 0){++dst;++p;--srclen;--dstlen;}
    if (reslen)
        *reslen = (p-src >= 0) ? (p-src)*sizeof(WCHAR) : dstlen; /* overflow -> we filled up to dstlen */
    return STATUS_SUCCESS;
}
 ...
 *  NTSTATUS code
 */
NTSTATUS WINE_WINAPI RtlUnicodeToMultiByteN( LPSTR dst, DWORD dstlen, LPDWORD reslen,
                                        LPCWSTR src, DWORD srclen )
{
	WCHAR *p = src;
	while(srclen && dstlen && (*dst = *p) != 0){++dst;++p;--srclen;--dstlen;}
	if (reslen)
		*reslen = (p-src >= 0) ? (p-src) : dstlen; /* overflow -> we filled up to dstlen */
	return STATUS_SUCCESS;
}
 ...
 */
CHAR WINE_WINAPI RtlUpperChar( CHAR ch )
{
    if (ch >= 'a' && ch <= 'z') {
        return ch - 'a' + 'A';
    } else {
 ...
 */
void WINE_WINAPI RtlUpperString( STRING *dst, const STRING *src )
{
 ...
 *  The uppercase character value.
 */
WCHAR WINE_WINAPI RtlUpcaseUnicodeChar( WCHAR wch )
{
    return toupperW(wch);
}
 ...
 */
WCHAR WINE_WINAPI RtlDowncaseUnicodeChar(WCHAR wch)
{
    return tolowerW(wch);
}
 ...
 */
NTSTATUS WINE_WINAPI RtlUpcaseUnicodeString( UNICODE_STRING *dest,
                                        ...
                                        BOOLEAN doalloc)
{
    ...
    if (doalloc)
    {
        ...
        if (!(dest->Buffer = malloc( len + sizeof(WCHAR))))
            return STATUS_NO_MEMORY;
    }
 ...
 *  might not be '\0' terminated. dest->Length is only set upon success.
 */
NTSTATUS WINE_WINAPI RtlDowncaseUnicodeString(
    UNICODE_STRING *dest,
    const UNICODE_STRING *src,
    BOOLEAN doalloc)
{
 ...
 */
NTSTATUS WINE_WINAPI RtlUpcaseUnicodeStringToAnsiString( STRING *dst,
                                                    const UNICODE_STRING *src,
                                                    BOOLEAN doalloc )
{
    NTSTATUS ret;
 ...
 *  NTSTATUS code
 */
NTSTATUS WINE_WINAPI RtlUpcaseUnicodeToMultiByteN( LPSTR dst, DWORD dstlen, LPDWORD reslen,
                                              LPCWSTR src, DWORD srclen )
{
    NTSTATUS ret;
    LPWSTR upcase;
    ...
    if (!(upcase = malloc( srclen ))) return STATUS_NO_MEMORY;
 ...
 *  The calculated size.
 */
UINT WINE_WINAPI RtlOemStringToUnicodeSize( const STRING *str )
{
 ...
 *  The calculated size.
 */
DWORD WINE_WINAPI RtlAnsiStringToUnicodeSize( const STRING *str )
{
    DWORD ret;
 ...
 *  STATUS_SUCCESS.
 */
NTSTATUS WINE_WINAPI RtlMultiByteToUnicodeSize( DWORD *size, LPCSTR str, UINT len )
{
 ...
 *  STATUS_SUCCESS.
 */
NTSTATUS WINE_WINAPI RtlUnicodeToMultiByteSize( PULONG size, LPCWSTR str, ULONG len )
{
    *size = len;
    return STATUS_SUCCESS;
}
 ...
 *  The calculated size.
 */
DWORD WINE_WINAPI RtlUnicodeStringToAnsiSize( const UNICODE_STRING *str )
{
    DWORD ret;
 ...
 */
NTSTATUS WINE_WINAPI RtlAppendAsciizToString(
 ...
 *  dest is never '\0' terminated.
 */
NTSTATUS WINE_WINAPI RtlAppendStringToString(
    STRING *dest,       /* [I/O] Buffered character string to which src is concatenated */
 ...
 */
NTSTATUS WINE_WINAPI RtlAppendUnicodeToString(
 ...
 *  MaximumLength is odd as the native function does.
 */
NTSTATUS WINE_WINAPI RtlAppendUnicodeStringToString(
    UNICODE_STRING *dest,      /* [I/O] Buffered unicode string to which src is concatenated */
 ...
 */
NTSTATUS WINE_WINAPI RtlFindCharInUnicodeString(
    int flags,                          /* [I] Flags */
 ...
 */
BOOLEAN WINE_WINAPI RtlIsTextUnicode( LPCVOID buf, INT len, INT *pf )
{
 ...
 */
NTSTATUS WINE_WINAPI RtlCharToInteger(
 ...
 */
NTSTATUS WINE_WINAPI RtlIntegerToChar(
 ...
 *  version does.
 */
NTSTATUS WINE_WINAPI RtlUnicodeStringToInteger(
 ...
 */
NTSTATUS WINE_WINAPI RtlIntegerToUnicodeString(
    ULONG value,         /* [I] Value to be converted */
    ULONG base,          /* [I] Number base for conversion (allowed 0, 2, 8, 10 or 16) */
 ...
 */
NTSTATUS WINE_WINAPI RtlGUIDFromString(PUNICODE_STRING str, GUID* guid)
{
  int i = 0;
 ...
 */
//NTSTATUS WINE_WINAPI RtlStringFromGUID(const GUID* guid, UNICODE_STRING *str)
//{
