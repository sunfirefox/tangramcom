...
//
//static int utli_strnicmp(const char *s1, const char *s2,int len){
//	while (len && *s1 && utli_lowpper(*s1) == utli_lowpper(*s2)){
//		len--;s1++;s2++;
//	}
//	return len == 0? 0 : utli_lowpper(*s1) - utli_lowpper(*s2);
//}
	...
	char *name)                /* [I] The encoded name to find. */
{
	...
	if (!((namestruct[2] ^ *((int *)name)) & 0xffff00ff)) {
	    ...
	    if (!widl_strcmpin(name+4, (void *)(namestruct+3), name[0])) break;
	}
...
static void add_interface_typeinfo(msft_typelib_t *typelib, type_t *__interface);
...
enum TYPEKIND;
static msft_typeinfo_t *create_msft_typeinfo(msft_typelib_t *typelib, /*enum*/ TYPEKIND kind,
...
static void add_interface_typeinfo(msft_typelib_t *typelib, type_t *__interface)
{
    ...
    if (-1 < __interface->typelib_idx)
        return;

    if (is_attr(__interface->attrs, ATTR_DISPINTERFACE))
        return add_dispinterface_typeinfo(typelib, __interface);

    /* midl adds the parent interface first, unless the parent itself
       has no parent (i.e. it stops before IUnknown). */

    if(__interface->ref) {
        ref_importinfo = find_importinfo(typelib, __interface->ref->name);

        if(!ref_importinfo && __interface->ref->ref && __interface->ref->typelib_idx == -1)
            add_interface_typeinfo(typelib, __interface->ref);
    }

    __interface->typelib_idx = typelib->typelib_header.nrtypeinfos;
    msft_typeinfo = create_msft_typeinfo(typelib, TKIND_INTERFACE, __interface->name, __interface->attrs);
    msft_typeinfo->typeinfo->size = 4;
    msft_typeinfo->typeinfo->typekind |= 0x2200;

    for (derived = __interface->ref; derived; derived = derived->ref)
    ...
    if(__interface->ref)
        add_impl_type(msft_typeinfo, __interface->ref, ref_importinfo);

    /* count the number of inherited interfaces and non-local functions */
    for(ref = __interface->ref; ref; ref = ref->ref) {
        num_parents++;
        if (ref->funcs)
    ...
    if (__interface->funcs)
        LIST_FOR_EACH_ENTRY( func, __interface->funcs, const func_t, entry )
            if(add_func_desc(msft_typeinfo, func, idx) == S_OK)
                idx++;
}
...
static void add_coclass_typeinfo(msft_typelib_t *typelib, type_t *cls)
{
    ...
    if (cls->ifaces) LIST_FOR_EACH_ENTRY( iref, cls->ifaces, ifref_t, entry ) {
        ...
        /* If the __interface is non-restricted and we haven't already had one then
